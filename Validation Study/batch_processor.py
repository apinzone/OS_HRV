...
def analyze_with_chronos(file_path, edf_file):
    ...
    try:
        ...
        # Extract R-R intervals from ChronOS analyzer
        if 'rr_intervals' in analyzer.ecg_data and analyzer.ecg_data['rr_intervals']:
            rr_intervals_ms = analyzer.ecg_data['rr_intervals']
            # Use the existing td_peaks for timestamps (already calculated in seconds)
            r_peak_timestamps = analyzer.ecg_data['td_peaks'].tolist()
        else:
            print(f"  ERROR: R-R intervals not found in analyzer")
            return None
        ...
        if hasattr(analyzer, 'results') and analyzer.results:
            ...
            if hrv_data:
                result = {
                    ...
                    'pNN50_%': hrv_data.get('pnn50', np.nan),
                    ...
                    'r_peak_timestamps_sec': r_peak_timestamps,
