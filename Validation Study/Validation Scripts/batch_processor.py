...
def analyze_with_neurokit(ecg_signal, sampling_rate, edf_file):
    ...
    try:
        ...
        # Calculate time domain metrics using NeuroKit2
        time_domain = nk.hrv_time(r_peaks, sampling_rate=sampling_rate, show=False)
        
        # Calculate nonlinear metrics using NeuroKit2 (includes sample entropy)
        nonlinear = nk.hrv_nonlinear(r_peaks, sampling_rate=sampling_rate, show=False)
        
        # Calculate frequency domain metrics using NeuroKit2 with 4Hz interpolation
        # CRITICAL: Set normalize=False to get absolute power in ms² units like ChronOS
        frequency_domain = nk.hrv_frequency(
            r_peaks, 
            sampling_rate=sampling_rate,
            interpolation_rate=4,  # Force 4Hz instead of default 100Hz
            normalize=False,       # Get absolute power units (ms²) not normalized
            show=False
        )
        ...
        # Nonlinear metrics from NeuroKit2 (includes sample entropy)
        ...
        # Sample entropy from NeuroKit2 (uses same parameters as ChronOS: m=2, r=0.2*std(ddof=1))
        sample_entropy = nonlinear['HRV_SampEn'].iloc[0] if 'HRV_SampEn' in nonlinear.columns else np.nan
        
        # Frequency domain metrics from NeuroKit2 (with 4Hz interpolation)
